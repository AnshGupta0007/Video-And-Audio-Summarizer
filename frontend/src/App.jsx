...
export default function App() {
  ...
  const handleAudioPipeline = async (audioFilePath) => {
    try {
      addProgress("Transcribing (original language)...");
      ...
      await handleTextPipeline(
        transNative.text_file,
        transNative.transcription,
        transEnglish.transcription
      );
    } catch (e) {
      console.error(e);
      addProgress("Error: " + e.message);
  ...
  const handleTextPipeline = async (
    ...
    try {
      ...
      addProgress("Creating audio (original)...");
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">
                Upload media or paste text / YouTube link
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-5">
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`${modeButtonBase} ${
                  mode === "full" ? modeFullActive : modeFullInactive
                }`}
              >
                Full Mode — 8 outputs
              </button>
              ...
              <button
                ...
                className={`${modeButtonBase} ${
                  mode === "fast" ? modeFastActive : modeFastInactive
                }`}
              >
            ...
            <p className="text-xs text-gray-500 mt-2">
              {mode === "full"
                ? "Creates audio, fast audio, and summaries."
                : "Quick summaries only. No audio."}
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            {inputType === "text" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 mt-2">
                    Upload a .txt file to summarize it.
                  </div>
                </div>
              </div>
            )}
            ...
            {inputType === "youtube" && (
              <div className="p-3 border rounded">
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Works with normal, short, and youtu.be links.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white mt-4 rounded-xl shadow p-3">
            ...
            <div className="max-h-36 overflow-auto text-sm space-y-1">
              {progress.map((p, i) => (
                <div key={i}>
                  • {p.msg}{" "}
                  <span className="ml-2 text-gray-400 text-xs">{p.time}</span>
                </div>
              ))}
            </div>
          </div>
...
function AudioBlock({ title, file, openServerFile }) {
  const src = file
    ? `${API_BASE}/files/${encodeURIComponent(file)}`
    : "";
